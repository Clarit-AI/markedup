...
| `summary`            | `Summary`          | `string`           | One-sentence entity description (generated by Tier 2 enrich) |
...
- **Required**: No
- **Validation**: None
- **Example**: `"AI researcher specializing in knowledge graphs and distributed systems."`

A one-sentence description of the entity or concept this page represents. Generated by the Tier 2 model-assisted enrichment pipeline (`enrich.GenerateSummary()`). The summary focuses on the entity itself, not the document structure.

The `summary` field is included in `CompactGraphSummary` output (used by `markedup_get_structure` and `export --compact`), providing LLMs with a quick description of each node without loading the full body text.
...
provenance:
  sources: ["hr-directory", "research-portal"]
  created-by: system
summary: "AI researcher specializing in knowledge graphs and distributed systems."
